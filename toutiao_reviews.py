...
# 设置user-agent
def set_ops():
    opts, ua = Options(), generate_user_agent()
...
class Driver(object):
    ...
    def login(self, url=None):
        global cd, wait,ac
        cd = webdriver.Chrome('../chromedriver.exe', chrome_options=Driver.set_ops())
        ...
        if len(self.username) == 0 or len(self.password) == 0:
            ...
            # login
            cd.find_element_by_xpath(self.username[0]).send_keys(name[i])
            cd.find_element_by_xpath(self.password[0]).send_keys(password[i])
            cd.find_element_by_xpath(self.submit).click()
            return cd,wait


def login(web = 'https://www.appannie.com/apps/ios/app/529092160/details/',):
    driver = Driver(landing_page='https://www.appannie.com/account/login/?_ref=header',
                    username=[".//*[@id='email']", 'username'],
...
class Vision:
    ...
    def get_each_visions(self):
        ...
        self.vision_df.to_csv('/Users/macbookpro/Desktop/Python/crawler/Task2/%svision.csv' % self.app_name,
                              encoding='utf-8')  # 获取review信息的
...
class review:
    ...
    def page_swich(self):
        ...
        time.sleep(uniform(1.0, 2.0))  # 选择每页显示200项
        wait.until(lambda cd: cd.find_element_by_link_text("Rating"))
        ...
        # 切换到断点页面
        self.next_reviews_page(beg)  # 获取总共的页码, 还有ratings的信息的
    ...
    def get_text(tree, xpath):
        ...
        return [node.text for node in nodes]  # 获取每一页200个的reviews的信息


    def get_each_page_reviews(self):
        tree = etree.HTML(cd.page_source)  # 得到reivewer name, title, contents的一个list
        ...
        for i in range(len(reviewers_name)):
            ...
            self.reviews_df.loc[len(self.reviews_df)] = [self.app_name, reviews_country[i], reviews_date[i], rate,
                                                                 reviews_content[i],
                                                                 reviewers_name[i], reviews_title[i], reviews_vision[i]]  # 切换页码
    ...
    def next_reviews_page(self, num):
        ...
        i_am_waiting_for_you(
            ...
            ".//*[@id='aa-app']/div/div/div[2]/div[1]/div[3]/div[2]/div/div[1]/div/div[1]")
        time.sleep(uniform(0.3, 0.423))  # 存储爬取的信息
    ...
    def save_reviews_info(self, name):
        ...
        self.reviews_df.to_excel('/Users/macbookpro/Desktop/Python/crawler/Task2/' + name + '.xlsx',encoding='utf-8')  # 获取所有页码的reviews信息
    ...
    def get_all_pages_reviews(self):
        for i in range(self.begin_page + 1, self.page_num + 1):
            ...
            time.sleep(uniform(0.7, 1.5))  # 用于翻页
